...
import './BaseRelayRecipient.sol';
 ...
 contract SportsDapp1 is BaseRelayRecipient{
    ...
    //  EIP712MetaTxn(name,  version,  chainid) {
        admin = _admin;
    ...
    function postQuestion(uint _qid, string memory _qcontent,
                             uint _vote_price, uint8  _optionslength, uint _type, string memory name ) public{
    ...
    function stopVoting(uint _qid) public{
        ...
        require(
            _msgSender() == admin,
            "Only admin can stop the voting"
        );
    ...
    function submitRightSolution(uint _qid, uint _ansid, string memory _correct_answer_str,string memory _correct_ans_reason) public{
        
        // based on live feed this should be updated
        require(
            _msgSender() == admin,
            "Only admin can submit the right solution"
        );
